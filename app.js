...
app.use(session({
  secret: 'keyboard cat', resave: false, saveUninitialized: false, maxAge: 5000000, store: new FileStore(),
}));
...
app.get('/backend/failure', (req, res) => {
  ...
  res.status(401).json({ success: false, errors: [{ param: 'username or password', msg: 'incorrect' }] });
});
...
app.post('/backend/login', passport.authenticate('local', { session: false, failureRedirect: '/backend/failure' }), loginHandler);
app.post('/login', passport.authenticate('local', { session: false, failureRedirect: '/failure' }), loginHandler);


app.use('/backend/api', passport.authenticate('jwt', { session: false }), apiRouter.getRouter);
app.use('/api', passport.authenticate('jwt', { session: false }), apiRouter.getRouter);
...
function loginHandler(req, res) {
  const SignedUser = { tokens: req.user.tokens, id: req.user.id, username: req.user.username };
